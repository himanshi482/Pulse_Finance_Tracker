...
function renderSnapshotGauge() {
  const percentVal = $('#snapshotPercentVal');
  const circle = $('#snapshotGaugeCircle');
  const daysVal = $('#snapshotDaysVal');
  const daysLabel = $('#snapshotDaysLabel');
  const catAmount = $('#snapshotCatAmount');
  const catName = $('#snapshotCatName');
  const catIcon = $('#snapshotCatIcon');

  if (!percentVal || !circle) return;

  const { expense } = totals();
  const budget = state.budget || 10000;
  
  // Calculate percentage spent vs monthly goal
  const usedPct = state.budget ? Math.round((expense / state.budget) * 100) : 103;
  percentVal.textContent = `${usedPct}%`;

  // SVG Circumference: 2 * Math.PI * 62 = ~389.55
  ...
  // If over 100%, set ring color to Coral Red (#d9534f), else Dark Mint (#1c563d)
  if (usedPct > 100) {
    circle.style.stroke = '#d9534f';
  } else {
    circle.style.stroke = '#1c563d';
  }

  // Top category spending calculation
  const spentTx = state.transactions.filter((x) => x.type === 'expense');
  const grouped = spentTx.reduce((acc, x) => {
  ...
  if (topCategoryEntry) {
    const [name, amt] = topCategoryEntry;
    if (catAmount) catAmount.textContent = money(amt);
    if (catName) catName.textContent = `${name.toLowerCase()} this month`;
    if (catIcon) catIcon.textContent = catIcons[name] || '☕';
  } else {
    if (catAmount) catAmount.textContent = '₹5,000';
    if (catName) catName.textContent = 'food & dining this month';
    if (catIcon) catIcon.textContent = '☕';
  }

  // Calculate days remaining in month or under budget status
  const today = new Date();
  const daysInMonth = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
  const remainingDays = daysInMonth - today.getDate();
  
  if (daysVal) {
    daysVal.textContent = `${remainingDays > 0 ? remainingDays : 18} days`;
  }
  if (daysLabel) {
    daysLabel.textContent = expense > budget ? 'over budget' : 'under budget';
  }
}
